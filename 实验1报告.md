...
├── 截图_任务2_GitHub仓库.png       # 任务2 仓库同步截图
└── samp2_4App/                    # Qt 工程源码
```
...
GitHub 账号：**DGLZR**，远程仓库：

```
https://github.com/DGLZR/QT-Homework1.git
```
...
同步结果核对（`git ls-remote origin`）：

| 位置 | 提交号 |
| --- | --- |
| 本地 `main` | `c63b53c` |
| 远程 `origin/main` | `c63b53c` |

两边完全一致，远程仓库共 **26 个文件 / 3 次提交**。

**任务2 截图**：`实验1/截图_任务2_GitHub仓库.png`

> 说明：该截图是按远程仓库 `origin/main` 的真实状态（文件名取自
> `git ls-tree -r origin/main`，提交记录取自 `git log`）渲染的仓库页面图，
> 页面上标注的 `26 files / 3 commits` 与 `main = c63b53c` 都是实际值。
> 因为该仓库当前是 **Private**，未登录的浏览器直接打开
> `github.com/DGLZR/QT-Homework1` 会返回 404，所以无法直接截取网页。
> 若把仓库改为 Public（Settings → General → 最下方 Change visibility），
> 就能直接截取真实的 GitHub 页面替换此图。
